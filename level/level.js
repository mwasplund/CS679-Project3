     ...
     window.addEventListener('load', function () {
		...
		context.clearRect(0, 0, canvas.width, canvas.height);
		 ...
		 this.mouseup = function (ev) {
			 ...
			 if(diffX < diffY){
			    polyArray[polyArray.length] = { 'xStart': tool.x0, 'yStart': tool.y0, 'xFinish': tool.x0, 'yFinish': ev._y };
			    
			    tool.y0 = ev._y;
			    
			 }else if(diffY <diffX){
			     polyArray[polyArray.length] = { 'xStart': tool.x0, 'yStart': tool.y0, 'xFinish': ev._x, 'yFinish': tool.y0 };
