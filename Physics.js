...
function createNumberEffect(val, src, start, end, isPlayer) {
	var effect = {};
	effect.src = src;
	effect.start = start;
	effect.end = end;
	...
	effect.drawGL = function() {
		glNumbers.addNumber(this.val, this.src.position || this.src, (tick - this.start) / (this.end - this.start), this.isPlayer ? [1, 0, 0] : [0, 1, 0]);
	}
	addEffect(effect);
	return effect;
}
