...
function FBX_Parser_ParseLayerElementNormal(i_FileContainer)
{
  ...
  if(i_FileContainer.HasNext)
  {
    ...
    {
      Debug.Trace("FBX ERROR: LayerElementNormal: not formatted correctly");
      return null;
    }
    ...
    {
      Debug.Trace("FBX ERROR: The LayerElementNormal: number was NaN");
      return null;
    }
  }
  ...
  while(i_FileContainer.HasNext)
  {
    ...
    else if(CurrentLine[0] == "MappingInformationType:")
    {
      ...
      {
      	Debug.Trace("FBX ERROR: LayerElementNormal MappingInformationType was formatted incorrectly");
        return null;
      }
    }
    else if(CurrentLine[0] == "ReferenceInformationType:")
    {
      ...
      {
      	Debug.Trace("FBX ERROR: LayerElementNormal ReferenceInformationType was formatted incorrectly");
        return null;
      }
    }
    ...
    {
      Debug.Trace("FBX ERROR: Found Unknown Token in LayerElementNormal: " + CurrentLine[0]);
      return null;
    }
  }
  
  Debug.Trace("FBX ERROR: Never saw the end bracket for LayerElementNormal:");
  return null;
}
...
function  FBX_Parser_ParseNormals(i_FileContainer)
{
  ...
  if(i_FileContainer.HasNext)
  {
    ...
    {
      Debug.Trace("FBX ERROR: Normals: not formatted correctly");
      return null;
    }
    ...
    if(isNaN(Count))
    {
      Debug.Trace("FBX ERROR: The Normals: count was NaN");
      return null;
    }
    if((Count % 3) != 0)
    {
      // There was an invalid number of numbers
      Debug.Trace("FBX ERROR: The Normals: count was not a multiple of three.");
      return null;
    }
  }
  ...
  if(i_FileContainer.HasNext)
  {
    ...
    for(var i = 1; i < SecondLine.length; i++)
    {
      ...
      if(isNaN(Value))
      {
        Debug.Trace("FBX ERROR: A Normal was " + Value);
        return null;
      }
      Normals.push(Value);
    }
  ...
  while(i_FileContainer.HasNext)
  {
    ...
    if(CurrentLine[0] == "}")
    {
    	...
    	{
    		Debug.Trace("FBX ERROR: Expected " + Count + " Normals but found " + Normals.length);
    		return null;
    	}
    	else
    	{
    ...
    for(var i = 0; i < CurrentLine.length; i++)
    {
      ...
      if(isNaN(Value))
      {
        Debug.Trace("FBX ERROR: A Normals was " + Value);
        return null;
      }
      Normals.push(Value);
    }
   ...
   Debug.Trace("FBX ERROR: Never saw the end bracket for Normals:");
   return null;
}
